...
from paddle_lpips.trainer import *
from paddle_lpips.lpips import *
...
#         Output paddlepaddle Variable N long
#         """
...
def tensor2np(tensor_obj):
    ...
    return tensor_obj[0].astype('float32').numpy().transpose((1,2,0))

def np2tensor(np_obj):
     # change dimenion of np array into tensor array
    return paddle.to_tensor(np_obj[:, :, :, np.newaxis].transpose((3, 2, 0, 1)))
...
def tensor2im(image_tensor, imtype=np.uint8, cent=1., factor=255./2.):
    image_numpy = image_tensor[0].astype('float32').numpy()
    image_numpy = (np.transpose(image_numpy, (1, 2, 0)) + cent) * factor
    return image_numpy.astype(imtype)

def im2tensor(image, imtype=np.uint8, cent=1., factor=255./2.):
    return paddle.to_tensor((image / factor - cent)
                        [:, :, :, np.newaxis].transpose((3, 2, 0, 1))).astype('float32')

def tensor2vec(vector_tensor):
    return vector_tensor.numpy()[:, :, 0, 0]
